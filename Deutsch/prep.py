...
verbs = [
    ...
    # Add more to reach 50+ verbs
]

score = 0
wrong_answers = []

print("German Verb + Preposition Quiz!\nType the correct preposition and case (Akk/Dativ).\n")

for _ in range(10):  # quiz length

    q = random.choice(verbs)
    print(f"Verb: {q['verb']}")
    prep_answer = input("Preposition: ").strip().lower()

    case_answer = input("Case (Akk/Dativ): ").strip().capitalize()
    if case_answer=="A":
        case_answer="Akk"
    if case_answer=="D":
        case_answer="Dat"
    ...
    if prep_answer == q["prep"] and case_answer == q["case"]:
        print("✅ Correct!\n")
        score += 1
    else:
        print(f"❌ Wrong. Correct: {q['verb']} {q['prep']} ({q['case']})")
        wrong_answers.append(f"{q['verb']} {q['prep']} ({q['case']}) → {q['translation']}")
    
    print(f"Translation: {q['translation']}\n")

print(f"Your score: {score}/10\n")

if wrong_answers:
    print("Review the verbs you missed:")
